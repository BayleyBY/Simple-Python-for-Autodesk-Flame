...
Description:
Configurable one-click new-project setup. Customize the number of reels on
the Desktop, their color and names. Rename the default library, create a
Reel Group with custom reels and sequence reels. Load a set of bookmarks.
The first time any action is used, a setup window asks how the project
should be laid out — desktop reels, library naming, an online reel group,
and a bookmarks template file — and saves the answers to a JSON file next
to the script. Every action (or All The Things) then runs with the saved
settings. Change the settings anytime with Setup... in the same menu.
