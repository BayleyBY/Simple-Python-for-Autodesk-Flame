...
Description:
Configurable one-click new-project setup. The first time any action is used,
a setup window asks how the project should be laid out — desktop reels,
library naming, an online reel group, and a bookmarks template file — and
saves the answers to a JSON file next to the script. Every action (or All
The Things) then runs with the saved settings. Change the settings anytime
