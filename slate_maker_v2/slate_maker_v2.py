...
Version History:
    ...
    v2.5.4 08.05.26 - Confirmation dialogs are now conditional: Rename from
    ...
    v2.5.3 08.05.26 - Fix: Update Date now treats DATE and CURRENT_DATE as the
    ...
    v2.5.2 08.05.26 - Fix: values read back via get_metadata() render wrapped
    ...
    v2.5.1 08.05.26 - Fix: the Metadata timeline effect's create_effect type
    ...
    v2.5.0 08.05.26 - Slate metadata is now stored as custom keys on a Metadata
    ...
    v2.4.2 08.05.26 - All update tools moved to their own 'Slate Maker: Update...'
    ...
    v2.4.1 08.05.26 - Context menu reorganized: all tools now live under one
    ...
    v2.4.0 08.05.26 - Update Field now works on any slate, not just v2-created ones.
    ...
    v2.3.0 08.05.26 - Added Update Field submenu: one menu item per configured field
    ...
    v2.2.0 08.02.26 - Update Slates now also works on sequences containing a v2 slate:
                      the slate segment is located by its tags and updated in place.
    v2.1.0 08.02.26 - Added Rename from Slate mode: rename sequences containing a v2 slate
                      (or slate clips themselves) from their slate token values using a
                      tokenized name pattern.
    v2.0.0 08.02.26 - Added Update Slates mode: token values are stamped to clip tags at
                      creation and can be bulk-edited later via Slate Maker: Update Slates.
    v1.0.0 04.29.25 - Initial version, derived from Uber Slate Maker with update
...
Menus:
    ...
    Slate Maker... -> Rename from Slate         (select sequences containing a slate, or slate clips)
    Slate Maker: Update... -> Update Slates     (bulk editor; v2 slate clips or sequences containing one)
    Slate Maker: Update... -> Update <Field>    (one item per learned/configured field; works on any slate)
    Slate Maker: Update... -> Edit Update Fields (curate the Update <Field> menu list)
...
import flame
from lib.pyflame_lib_slate_maker_v2 import *
